angularjs-gulp-browserify-bootstrap
=====================================
...
Running  gulp prod from the application will create a production folder under build folder which will contain the
1. css Folder inside will find main.css.gz file
2. index.html.gz file
3. js Folder inside will find 2 gzip files main.js.gz and main.js.map.gz

copy file/folder to remote server, make sure you on the local machine inside the containing folder

 rsync -avz build devroot@dchop35.standardbank.co.za:/home/devroot/

ssh  devroot@dchop35.standardbank.co.za
...
sudo rm -rf build/ css/ images/ index.html  js/ fonts/

sudo cp -r /home/devroot/build/ .

sudo cp -r build/images/ build/index.html build/js/ build/fonts/ build/css/ .
...
sudo unzip supportui.zip -d html/


Using Chef knife to deploy
==========================

SIT 2
=====

knife job start --quorum 100% 'chef-client' --search 'chef_environment:supportui-sit AND department:MCAP AND description:SupportUI'

Production
==========
knife job start --quorum 100% 'chef-client' --search 'chef_environment:supportui-prd AND role:supportui-prd'


testing modals examples
